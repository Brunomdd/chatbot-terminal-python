...
Sistema de chatbot em terminal que utiliza modelos locais via Ollama, permitindo interação com IA diretamente no terminal.

O projeto salva histórico das conversas em arquivo JSON para persistência.
...
├── historico_bot.json
🔄 Fluxo do sistema
1. Usuário escolhe modelo
2. Usuário digita pergunta
3. Sistema monta JSON:
   {
     model, prompt, stream
   }

4. Envia POST para Ollama
5. Recebe resposta da IA
6. Extrai "response"
7. Salva no histórico JSON
8. Mostra conversa no terminal
...
💡 Melhorias futuras (ideias de evolução)
